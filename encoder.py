...
class GRUEncoder(nn.Module):
    def __init__(self, word2id, tokenizer, hidden_size = 100, pretrained_vocab=None, pretrained_emb = None):
        ...
        self.embed = nn.Embedding(num_embeddings = self.embed_matrix.shape[0],
                embedding_dim = self.embed_matrix.shape[1], padding_idx = 0)
    ...
    def init_embeddings(self, pretrained_vocab, pretrained_emb):
        ...
        uninitialized = [words for words in self.word2id.values() if not words in self.word_embed]
    ...
    def prepare_batch(self, texts):#gets a list of untokenized texts, returns a padded batch that can be used as an input to forward, puts it on the same device as the GRU
        ...
        for text in texts:
            if text in self.tokenization_memo:
                ...
                tokenized = [self.word2id[token.text.lower()] for token in self.tokenizer(text,disable=['parser','tagger','ner']) if token.text.lower() in self.word2id]
                self.tokenization_memo[text]=tokenized
                tokenized_texts.append(tokenized)
        padded_length = max(len(text) for text in tokenized_texts)
        phrase_batch = np.zeros((batch_size,padded_length),dtype=int)#<PAD> has id 0, that's why zeros
...
class NoEncoder(nn.Module):
    def __init__(self, name2id, hidden_size = 100, pretrained_encoder=None, add_inverse=False):
    ...
    def init_embeddings(self, pretrained_model):
        ...
        for i in range(0,len(all_keys),1000):
            encoded_batch,lens = pretrained_model.prepare_batch(all_keys[i:i+1000])
            nonzero_indices = torch.nonzero(lens,as_tuple=False).squeeze()#since unknown words are removed, we can get length 0 inputs. Remove to avoid crash, replace with random initializations later
            new_encoded_batch = torch.index_select(encoded_batch,0,nonzero_indices)
            new_lens = torch.index_select(lens,0,nonzero_indices)
            if len(new_lens)!=0:
                pretrained_embeddings = pretrained_model.forward(new_encoded_batch,new_lens)
                pretrained_embeddings = pretrained_embeddings.detach().cpu().numpy()
            nonzero_indices = nonzero_indices.cpu().numpy()
            cnt=0
            for j in range(len(lens)):
                if j in nonzero_indices:
                    self.embed_matrix[self.name2id[all_keys[i+j]]]=pretrained_embeddings[cnt]
                    cnt+=1
                else:
                    self.embed_matrix[self.name2id[all_keys[i+j]]]=np.random.normal(size=self.hidden_size)
    ...
    def prepare_batch(self, texts):#gets a list of untokenized texts, returns a padded batch that can be used as an input to forward, puts it on the same device as the GRU
        ...
        return torch.from_numpy(np.array([self.name2id[t] for t in texts])).to(device),None #Extra None is returned so that the same code works for "regular" encoder and this
